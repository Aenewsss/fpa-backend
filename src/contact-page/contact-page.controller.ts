...
export class ContactPageController {
    constructor(
        private readonly service: ContactPageService
    ) { }

    @Put()
    @ApiOperation({ summary: 'Atualizar página `contato`' })
    @ApiResponse({ status: 201, description: 'Página `contato` atualizada com sucesso' })
    @ApiBearerAuth()
    ...
    async create(@Body() dto: UpdatePageDto): Promise<StandardResponse> {
        ...
        return {
            message: ResponseMessageEnum.CONTACT_PAGE_UPDATED_SUCCESSFULLY,
            data: result
        }
    }

    @ApiOperation({ summary: 'Buscar a página `contato`' })
    @Get()
    async findOne(): Promise<StandardResponse> {
        const result = await this.service.findOne();
        return {
            message: ResponseMessageEnum.CONTACT_PAGE_FETCHED_SUCCESSFULLY,
            data: result
        }
    }
}
